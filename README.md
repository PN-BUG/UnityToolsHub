# Unity Tools Hub
<img width="1075" height="911" alt="img_v3_02158_d11bff3c-1cb2-4021-89b6-721052856e0g" src="https://github.com/user-attachments/assets/31a37b17-81a5-48ad-8c88-4a9615a33fd5" />
...
- **设置中心**：配置最近使用/最常使用快捷列表、显示数量、Unity 顶栏入口和隐藏项
...
- **显示控制边界**：批量隐藏只影响 Hub 左侧列表，不会禁用功能、改变第三方启用状态或卸载包；关闭开关时只恢复对应类型
...
- **批量显示控制**：隐藏项管理中的“隐藏第三方工具”只控制 Hub 列表可见性，不会修改这里的启用/禁用状态
...
- `window`：目标类继承 `EditorWindow`，Hub 直接打开窗口。
- `menu`：设置 `MenuItem`，Hub 调用已有 Unity 菜单入口。
- `static`：设置 `StaticMethod` 为无参数静态方法的完整名称。

SDK 的完整示例见 `SDK~/README.md`。
