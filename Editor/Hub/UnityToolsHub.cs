...
public partial class UnityToolsHub : EditorWindow
{
    ...
    private bool _dynamicMenuRefreshQueued;
    private static readonly List<string> DynamicMenuPaths = new List<string>();

    [InitializeOnLoadMethod]
    private static void InitializeDynamicUnityMenu()
    {
        EditorApplication.delayCall += () =>
        {
            if (HasOpenInstances<UnityToolsHub>())
            {
                GetWindow<UnityToolsHub>().QueueDynamicMenuRefresh();
                return;
            }
    ...
    {
        var json = EditorPrefs.GetString(HubSettingsPrefsKey, "");
        if (!string.IsNullOrEmpty(json))
        {
            try { _hubSettings = JsonUtility.FromJson<HubSettings>(json) ?? new HubSettings(); }
            catch { _hubSettings = new HubSettings(); }
        }
    ...
    public static void OpenRegisteredTool(string typeName)
    {
        ...
        hub.DiscoverTools();
        hub.OpenToolWindow(typeName);
        if (hub._toolIndex.TryGetValue(typeName, out var tool)) hub.RecordToolUsage(tool);
    }

    private void QueueDynamicMenuRefresh()
    {
        if (_dynamicMenuRefreshQueued) return;
        _dynamicMenuRefreshQueued = true;
        EditorApplication.delayCall += () =>
        {
            if (this == null) return;
            _dynamicMenuRefreshQueued = false;
            RefreshDynamicUnityMenu();
        };
    }

    private void RefreshDynamicUnityMenu()
    {
        var selected = _hubSettings.unityMenuTools
            .Select(typeName => _toolIndex.TryGetValue(typeName, out var tool) ? tool : null)
        ...
        var addMenuItem = menuType?.GetMethod("AddMenuItem", BindingFlags.Static | BindingFlags.NonPublic,
            null, new[] { typeof(string), typeof(string), typeof(bool), typeof(int), typeof(Action), typeof(Func<bool>) }, null);
        var removeMenuItem = menuType?.GetMethod("RemoveMenuItem", BindingFlags.Static | BindingFlags.NonPublic,
            null, new[] { typeof(string) }, null);
        if (addMenuItem == null || removeMenuItem == null)
        {
            Debug.LogWarning("[UnityToolsHub] 当前 Unity 版本不支持动态菜单注册，最近使用/最常使用菜单已跳过。");
            return;
        }

        foreach (var oldPath in DynamicMenuPaths)
            removeMenuItem.Invoke(null, new object[] { oldPath });
        DynamicMenuPaths.Clear();

        var uniquePaths = new HashSet<string>();
        Action<string, ToolEntry, int> register = (group, tool, priority) =>
        {
            string menuName = tool.name.Replace("/", "／");
            string path = $"UnityToolsHub/{group}/{menuName}";
            if (!uniquePaths.Add(path)) return;
            string typeName = tool.typeName;
            Action execute = () => OpenRegisteredTool(typeName);
            Func<bool> validate = () => true;
            addMenuItem.Invoke(null, new object[] { path, "", false, priority, execute, validate });
            DynamicMenuPaths.Add(path);
        };
        for (int i = 0; i < recent.Count; i++) register("最近使用", recent[i], -80 + i);
        for (int i = 0; i < mostUsed.Count; i++) register("最常使用", mostUsed[i], -60 + i);
        for (int i = 0; i < selected.Count; i++) register("工具", selected[i], 1000 + i);
    }
